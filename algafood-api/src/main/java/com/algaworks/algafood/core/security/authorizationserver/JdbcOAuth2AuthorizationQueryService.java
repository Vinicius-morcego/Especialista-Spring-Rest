...
public class JdbcOAuth2AuthorizationQueryService implements OAuth2AuthorizationQueryService{
	...
	private final String LIST_AUTHORIZED_CLIENTES = "select rc.* from oauth2_authorization_consent c "
			+ "inner join oauth2_registered_client rc on rc.id = "
			+ "c.registered_client_id where c.principal_name = ? ";
	
	private final String LIST_AUTHORIZED_QUERY = "select a.* oauth2_authorization a "
			+ "inner join oauth2_registered_client c on c.id = "
	...
	public List<RegisteredClient> listClientsWithConsent(String principalName) {
		return this.jdbcOperations.query(LIST_AUTHORIZED_CLIENTES, registeredClientRowMapper, principalName);		
	}

	@Override
	public List<OAuth2Authorization> listAuthorizations(String principalName, String clientId) {		
		return this.jdbcOperations.query(LIST_AUTHORIZED_QUERY, oAuth2AuthorizationRowMapper, principalName, clientId);
	}
